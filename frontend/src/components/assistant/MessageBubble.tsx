...
export function MessageBubble({ message, onFollowUpClick, onRetry }: MessageBubbleProps) {
  ...
  if (message.isError) {
    return (
      <div className="flex justify-start my-4 max-w-3xl">
        <div className="flex items-start space-x-3 w-full">
          <div className="w-8 h-8 rounded-xl bg-red-100 text-red-600 flex items-center justify-center shrink-0 mt-1 shadow-2xs">
            <AlertCircle className="w-4 h-4" />
          </div>
          <div className="bg-red-50/50 border border-red-200 rounded-2xl rounded-tl-xs p-4 flex-1">
            <div className="flex items-center space-x-2 text-red-800 font-semibold text-xs mb-1">
              <span>MANAK AI Response Error</span>
            </div>
            ...
            {onRetry && (
              <button
                ...
                className="inline-flex items-center space-x-1.5 px-3 py-1.5 rounded-lg text-xs font-medium bg-white border border-red-300 text-red-700 hover:bg-red-50 transition-colors shadow-2xs"
              >
  ...
  return (
    <div className="flex justify-start my-4 max-w-3xl">
      <div className="flex items-start space-x-3 w-full">
        <div className="w-8 h-8 rounded-xl bg-red-50 border border-red-200 text-red-600 flex items-center justify-center shrink-0 mt-1 shadow-2xs">
          <Sparkles className="w-4 h-4" />
        </div>

        <div className="bg-white border border-gray-200 rounded-2xl rounded-tl-xs p-5 shadow-2xs flex-1 space-y-4">
          {/* Header & Confidence */}
          ...
          {message.followUpPrompt && (
            <div className="bg-gray-50 border border-gray-200 rounded-xl p-3 flex items-start space-x-2 text-xs">
              ...
              <span className="text-gray-700 italic">"{message.followUpPrompt}"</span>
            </div>
          )}
